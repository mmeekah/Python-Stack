...
def success():
    mysql = connectToMySQL('SimplifiedWall')
    query = "SELECT * FROM users WHERE id=%(1)s;"
    data  = {
        'first_name': session['first_name']
    }
    ...
    mysql = connectToMySQL('SimplifiedWall')
    query = mysql.query_db("SELECT users.first_name, messages.id, messages.message, messages.created_at FROM users JOIN messages on messages.sender_id = users.id WHERE messages.recipient_id = %(recipient_id)s")
    print("QUERY", query)
    session['id'] = query.id
    data = {
